...
- (void)setupPreviewLayer {
  ...
  if (sharedChecker) {
    // Shared mode: RNFibriCheckView is mounted — piggyback on its capture session.
    [sharedChecker startPreview];
    AVCaptureSession *session = sharedChecker.captureSession;
    if (!session) {
      RCTLogWarn(@"[RNCameraPreviewView] captureSession unavailable after startPreview");
      return;
    }
